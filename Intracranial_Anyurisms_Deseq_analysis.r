...
# Subset data by experimental group
...
dat.ia     <- dat.table[, dat.factor %in% c("unruptured intracranial aneurysm", 
                                            "ruptured intracranial aneurysm")]

# Calculate average gene expression per group
dat.rupt.m   <- apply(dat.rupt, 1, mean, na.rm = TRUE)
dat.unrupt.m <- apply(dat.unrupt, 1, mean, na.rm = TRUE)
dat.sta.m    <- apply(dat.sta, 1, mean, na.rm = TRUE)
dat.ia.m     <- apply(dat.ia, 1, mean, na.rm = TRUE)
...
leg <- seq(min(dat.cor, na.rm = TRUE), max(dat.cor, na.rm = TRUE), length = 10) 

image(dat.cor, main = "Correlation plot Normal/Tumor data", axes = FALSE, col = cx) 
...
plot(dat.mean, dat.cv,
     main = "Intracranial Aneurysms dataset CV vs. Mean",
...
plot(c(1, length(dat.avg)), range(dat.avg), type = "n",
     xlab = "", ylab = "Avg r", main = "Avg correlation of Tumor/Normal samples", axes = FALSE) 
points(dat.avg, bg = "red", col = 1, pch = 21, cex = 1.25) 
axis(1, at = 1:length(dat.avg), labels = dimnames(dat.table)[[2]], las = 2, cex.lab = 0.4, cex.axis = 0.6) 
axis(2) 
...
# Student’s t-tests between groups
...
# P-value histograms
hist(pv.sta.ia, col = "lightblue", xlab = "p-values", 
     main = "P-value dist’n between\nControl and IA groups", cex.main = 0.9)
hist(pv.unrupt.rupt, col = "lightblue", xlab = "p-values", 
...
# Significant results after filtering
...
# Volcano plot: STA vs IA
plot(range(pv.trans.sta.ia), range(fold.sta.ia), type = 'n',
...
points(pv.trans.sta.ia[(pv.trans.sta.ia > -log10(bonf.sta.ia) & fold.sta.ia > 1)],
       fold.sta.ia[(pv.trans.sta.ia > -log10(bonf.sta.ia) & fold.sta.ia > 1)], col = 1, bg = 2, pch = 21) 
points(pv.trans.sta.ia[(pv.trans.sta.ia > -log10(bonf.sta.ia) & fold.sta.ia < -1)],
       fold.sta.ia[(pv.trans.sta.ia > -log10(bonf.sta.ia) & fold.sta.ia < -1)], col = 1, bg = 3, pch = 21) 
abline(v = -log10(bonf.sta.ia)); abline(h = c(-1, 1))

# Volcano plot: UIA vs RIA
plot(range(pv.trans.unrupt.rupt), range(fold.unrupt.rupt), type = 'n',
     xlab = '-1*log10(p-value)', ylab = 'fold change', 
     main = 'Volcano Plot\nUnruptured and Ruptured group differences') 
points(pv.trans.unrupt.rupt, fold.unrupt.rupt, col = 'black', pch = 21, bg = 1) 
points(pv.trans.unrupt.rupt[(pv.trans.unrupt.rupt > -log10(0.05) & fold.unrupt.rupt > 1)],
       fold.unrupt.rupt[(pv.trans.unrupt.rupt > -log10(0.05) & fold.unrupt.rupt > 1)], col = 1, bg = 2, pch = 21)  
points(pv.trans.unrupt.rupt[(pv.trans.unrupt.rupt > -log10(0.05) & fold.unrupt.rupt < -1)],
       fold.unrupt.rupt[(pv.trans.unrupt.rupt > -log10(0.05) & fold.unrupt.rupt < -1)], col = 1, bg = 3, pch = 21) 
abline(v = -log10(0.05)); abline(h = c(-1, 1))
...
# Heatmaps
hm.rg <- c("#FF0000","#CC0000","#990000","#660000","#330000",
           "#000000","#000000","#0A3300","#146600","#1F9900",
           "#29CC00","#33FF00")

heatmap(as.matrix(dat.sig.sta.ia), col = hm.rg,
        xlab = "Samples", ylab = "Genes",
        main = "Heatmap of significantly regulated genes\nSTA vs IA data",
        cexCol = 0.5)

heatmap(as.matrix(dat.sig.unrupt.rupt), col = hm.rg,
        xlab = "Samples", ylab = "Genes",
        main = "Heatmap of significantly regulated genes\nUIA vs RIA data",
        cexCol = 0.5)

# PCA analysis
dat.pca.sta.ia       <- prcomp(t(dat.sig.sta.ia), cor = FALSE)
dat.pca.unrupt.rupt  <- prcomp(t(dat.sig.unrupt.rupt), cor = FALSE)

# PCA plots
dat.loadings <- dat.pca.sta.ia$x[, 1:2]
par(xpd = NA) 
plot(range(dat.loadings[,1]), range(dat.loadings[,2]), type = "n",
     xlab = 'p1', ylab = 'p2', main = 'PCA plot of Sotiriou Data\np2 vs. p1') 
...
par(xpd = NA) 
plot(range(dat.loadings2[,1]), range(dat.loadings2[,2]), type = "n",
     xlab = 'p1', ylab = 'p2', main = 'PCA plot of Sotiriou Data\np2 vs. p1') 
...
plot(1:length(dat.pca.var1), dat.pca.var1, type = "b", xlab = "# components", ylab = "% variance", 
     pch = 21, col = 1, bg = 3, cex = 1.5) 
...
plot(1:length(dat.pca.var2), dat.pca.var2, type = "b", xlab = "# components", ylab = "% variance", 
     pch = 21, col = 1, bg = 3, cex = 1.5) 
title("Scree plot showing % variability explained by each eigenvalue\nUIA/RIA dataset")

# LDA analysis and plots
...
plot(dat.pred$x, col = dat.out.factor, pch = as.numeric(dat.pred$class),
     ylab = "Discriminant function", xlab = "Score",
     main = "Discriminant function for STA/IA dataset") 
legend("bottomright", legend = c("predRIA","predUIA","predSTA","RIA","UIA","STA"),
       pch = c(1,3,2,21,21,21), pt.bg = c(1,3,2))

plot(dat.pred2$x, col = dat.ia.factor, pch = as.numeric(dat.pred2$class),
     ylab = "Discriminant function", xlab = "Score",
     main = "Discriminant function for UIA/RIA dataset")
legend("bottomright", legend = c("predRIA","predUIA","RIA","UIA"),
       pch = c(1,2,21,21), pt.bg = c(1,2))
